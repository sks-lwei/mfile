package ml.lwei.mfile.model.constant;
...
public class ZFileConstant {
     ...
     * 系统产生的临时文件路径
     */
    public static String TMP_FILE_PATH = "/.mfile/tmp2/";
    ...
    public void setTmpFilePath(@Value("${mfile.tmp.path}") String tmpFilePath) {
        ZFileConstant.TMP_FILE_PATH = tmpFilePath;
    }


    @Autowired(required = false)
    public void setHeaderFileName(@Value("${mfile.constant.readme}") String headerFileName) {
        ZFileConstant.README_FILE_NAME = headerFileName;
    }

    @Autowired(required = false)
    public void setPasswordFileName(@Value("${mfile.constant.password}") String passwordFileName) {
        ZFileConstant.PASSWORD_FILE_NAME = passwordFileName;
    }

    @Autowired(required = false)
    public void setAudioMaxFileSizeMb(@Value("${mfile.preview.audio.maxFileSizeMb}") Long maxFileSizeMb) {
        ZFileConstant.AUDIO_MAX_FILE_SIZE_MB = maxFileSizeMb;
    }

    @Autowired(required = false)
    public void setTextMaxFileSizeMb(@Value("${mfile.preview.text.maxFileSizeKb}") Long maxFileSizeKb) {
